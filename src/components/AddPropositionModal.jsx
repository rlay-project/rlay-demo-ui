...
class AddPropositionModal extends React.Component {
  ...
  handleSubmitClick = () => {
    ...
    const proposition = compactProposition(
      enrichPropositionInference(this.state.formProposition, ontologyClasses),
    );
  ...
  render() {
    ...
    return (
      <div>
        ...
        <ul>{explanation.asserted.map(expl => <li>{expl}</li>)}</ul>
        <b>Plaintext Explanation (inferred):</b>
        <ul>{explanation.inferred.map(expl => <li>{expl}</li>)}</ul>
        <b>RDF equivalent:</b>
        <ul>
