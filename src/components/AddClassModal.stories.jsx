  ...
  return class extends React.Component {
    ...
    componentDidMount() {
      bayModulePromise.then((bayModule) => {
        const ontologyAnnotations = this.props.ontologyAnnotations
          .map(n => new Annotation(n));
        ontologyAnnotations.forEach(n => n.hash(bayModule));
        const ontologyClasses = this.props.ontologyClasses
          .map(n => new Class(n));
...
storiesOf('AddClassForm', module)
  .add('default', () => {
    ...
    const WrappedAddClassForm = withBayModuleAnnotations(AddClassForm, Rust.bay_web);
